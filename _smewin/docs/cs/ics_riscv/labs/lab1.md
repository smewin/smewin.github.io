...
当然，由于是 4 位整数相乘，如果结果也需要表示成 4 为整数的形式，我们就需要采取截断操作，只取后四位整数。但在这次实验中，我们可以用模拟的数据结构保留完整的大整数。
...
所以，我们的 8 位寄存器本质可以理解成两个 4 位寄存器，前 4 位是一个加法寄存器，后四位用作辅助存储乘数 B 和临时结果。
...
如果只是想实现一个正确的大整数运算，我们显然有一个十分简单的思路：用一个 64 位整数存放临时值（假设为 `tmp` ），建立一个循环，循环 `a` 次，每次计算 `tmp + b` 和 `m` 的大小关系，如果大于等于，则需执行 `tmp = tmp + b - m;` ，否则 `tmp = tmp + b;` 。（当然，你需要很小心地处理溢出情况。）
...
显然， Python 虽然支持大整数运算，但机器底层并不存在那么大的整数，感兴趣的同学可以搜索 Python 在底层是如何存储这些大整数的。对于大整数这类数据运算的效率问题，当然还是根据数学原理进行优化的效果最爽，可以尝试了解 Karatsuba 算法和 FFT 乘法。